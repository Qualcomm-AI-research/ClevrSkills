...
class Swap(Task):
    ...
    The goal is to swap the positions of two specified actors.
    If push is True, the actors must be swapped by pushing instead of picking-and-placing.
    """
    ...
    def setup_task(self):
        ...
        p1 = PredicateClass(self._env, actor_a, buff_pose)
        p2 = PredicateClass(self._env, actor_b, actor_a.pose)
        p3 = PredicateClass(self._env, actor_a, temp_pose)
        ...
        self.predicate = SwapPredicate("Swap the actors", [p1, p2, p3])
        ...
        if self._push:
            self.prompts = [prompt + " by pushing" for prompt in self.prompts]
