...
@register_task("NovelNounAdjective")
class NovelNounAdjective(PlaceOnTop):
    """
    ...
    The actor and the area are specified by combination of adjectives and nouns instead of images.
    """

    unsupported_task_args = ["sample_num_actors", "variant"]

    def __init__(self, env, sample_num_actors: bool = True, record_dir=None, split="train"):
        """
    ...
    def setup_task(self):
        ...
        self.prompts = [
            f"This is a {actor_noun} "
            f"{get_actor_description(self.main_actor_no_tex)}. "
            f"This is a {area_noun} "
            f"{get_actor_description(self.main_area_no_tex)}. "
            f"{examples}. Put the {adj} {actor_noun} on {area_noun}"
        ]
